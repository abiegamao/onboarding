...
function StatusMessage({ type, message }: { type: "success" | "error"; message: string }) {
    return (
        <div className={`flex items-center gap-2 rounded-lg px-4 py-3 text-sm ${type === "success" ? "bg-emerald-500/10 text-emerald-500 ring-1 ring-emerald-500/20" : "bg-destructive/10 text-destructive ring-1 ring-destructive/20"}`}>
...
export default function ClientSettingsPage() {
    ...
    return (
        <div className="container mx-auto max-w-2xl px-4 py-8 sm:px-6">
            ...
            <div className="flex gap-1 rounded-xl border border-primary/15 bg-card p-1.5 w-fit mb-6">
                {tabs.map((tab) => (
                    <button
                        key={tab.id}
