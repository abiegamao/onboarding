...
export async function GET(req: NextRequest) {
    ...
    const cacheKey = `admin:clients:${page}:${limit}:${phase ?? ""}:${staleOnly}:${search}:${sortField}:${sortDir}`
    ...
    const data = await withCache(cacheKey, 120, async () => {
        ...
        let clients = profiles.map((p) => {
            ...
            const progress = p.status.isCompleted
                ? 100
        ...
        if (sortField) {
            clients.sort((a, b) => {
                ...
                if (sortField === "name") {
                    ...
                    av = a.lastActive ? String(a.lastActive) : ""
                    bv = b.lastActive ? String(b.lastActive) : ""
                }
